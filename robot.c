...
int checkRobotSensorRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180 +PI/4)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180 +PI/4));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180 +PI/4)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180 +PI/4));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle)*PI/180 -PI/4)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180) -PI/4);
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2)*sin((robot->angle)*PI/180 -PI/4)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180 -PI/4));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-2)*cos((robot->angle)*PI/180 +PI/4)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180 +PI/4));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-2)*sin((robot->angle)*PI/180 +PI/4)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180 +PI/4));
        xTL = (int) xDir;
        yTL = (int) yDir;
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle)*PI/180 -PI/4)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180 -PI/4));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2)*sin((robot->angle)*PI/180 -PI/4)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180 -PI/4));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int left_sensor, int right_sensor) {
    ...
    if (((front_right_sensor>1) || (right_sensor>1)) && (wallfound == 0)){
        wallfound = 1;
        robot->direction = LEFT;
    }
    ...
    else if (right_sensor == 0 && wallfound ==1){
